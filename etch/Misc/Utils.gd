...
func calculte_bounding_boxes(strokes: Array[BrushStroke], margin: float = 0.0) -> Dictionary[BrushStroke, Rect2]:
	var result: Dictionary[BrushStroke, Rect2] = {}
...
func is_valid_etch_file(filepath: String) -> bool:
	return filepath.to_lower().ends_with(".etch") or filepath.to_lower().ends_with(".lorien")
...
func is_action_pressed(action_name: StringName, event: InputEvent) -> bool:
	return _does_action_match(action_name, event) and event.is_pressed()


func is_action_released(action_name: StringName, event: InputEvent) -> bool:
	return _does_action_match(action_name, event) and event.is_released()


func _does_action_match(action_name: StringName, event: InputEvent) -> bool:
	return InputMap.action_has_event(action_name, event) and not event.is_echo()
