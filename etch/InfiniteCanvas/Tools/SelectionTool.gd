...
const GROUP_SELECTED_STROKES = "selected_strokes"
# strokes that are in selection rectangle but not commit (i.e. the user is still selecting)
const GROUP_STROKES_IN_SELECTION_RECTANGLE = "strokes_in_selection_rectangle"
# strokes that need to be deslected once LMB is released
const GROUP_MARKED_FOR_DESELECTION = "strokes_marked_for_deselection"
...
var _stroke_positions_before_move: Dictionary[BrushStroke, Vector2] = {}
var _bounding_box_cache: Dictionary[BrushStroke, Rect2] = {}
...
func tool_event(event: InputEvent) -> void:
	...
	if copy_pressed or duplicate_pressed:
	...
	if paste_pressed or duplicate_pressed:
	...
	if event is InputEventMouseButton and not disable_stroke:
		if event.button_index == MOUSE_BUTTON_LEFT:
			...
			if event.pressed:
				...
				elif get_selected_strokes().is_empty():
					_state = State.SELECTING
		...
		elif event.button_index == MOUSE_BUTTON_RIGHT and event.pressed and _state == State.NONE:
			deselect_all_strokes()
	...
	for stroke: BrushStroke in _canvas.get_strokes_in_camera_frustrum():
		...
		if selection_rect.intersects(bounding_box):
			for point: Vector2 in stroke.points:
				if selection_rect.has_point(stroke.position + point):
...
func get_selected_strokes() -> Array[BrushStroke]:
	return Array(get_tree().get_nodes_in_group(GROUP_SELECTED_STROKES), TYPE_OBJECT, "Node2D", BrushStroke)
