...
func tool_event(event: InputEvent) -> void:
	...
	# Moving the tail
	elif event is InputEventMouseMotion and performing_stroke:
	...
	# Start + End
	elif not disable_stroke and event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT:
		if event.pressed:
			start_stroke()
