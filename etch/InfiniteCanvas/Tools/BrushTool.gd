...
func tool_event(event: InputEvent) -> void:
	...
	if event is InputEventMouseMotion:
		...
		if _current_pressure < 0 or is_zero_approx(_current_pressure):
	...
	elif event is InputEventMouseButton and not disable_stroke:
...
func _notification(what: int) -> void:
	if what == NOTIFICATION_WM_MOUSE_EXIT and performing_stroke:
		end_stroke()
